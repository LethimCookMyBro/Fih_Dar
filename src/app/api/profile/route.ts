...
import { getOrCreateCurrentProfile, updateCurrentProfile } from '@/server/profile-service';

export async function GET() {
  try {
    const context = await requireCurrentClerkUser();
    return NextResponse.json({ profile: await getOrCreateCurrentProfile(context) });
  } catch (error) {
